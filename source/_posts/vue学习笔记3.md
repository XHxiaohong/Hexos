...
## 前言
   最近一直没有动手写博客，一个是没有时间，另一个是感觉到了个人的不足，今天动手写这篇微博，一个完成vue系列的学习笔记，另一个是温习一下vue。
***
## 深入组件
   在上一篇中，我们已经接触到了组件，对于组件的应用有了初步的了解，而在一些复杂的场景，单单基础的组件很难满足需求，在这里将会对这类情况进行解决。

### 插槽
   **组件** 主要是将相同的功能提出来进行封装，在现实应用场景中，不同的页面上功能更多的是相似而不是完全相同，这种情况，单单用组件就无法满足需求了，如果用上插槽就可以完美的解决这类问题。
...
<!--more-->
### 动态组件和异步组件
   **动态组件** 在实际开发中，我们经常会用到选项卡之类的功能，虽然我们利用数据双向绑定也能可以满足，却不够优雅，而且有时候我们需要它们第一次被创建的时候缓存下来。
...
查看[完整代码](https://jsfiddle.net/chrisvfritz/Lp20op9o/)
   ...
   **异步组件** 在大型应用中，我们可能需要将应用分割成小一些的代码块，并且只在需要的时候才从服务器加载一个模块。（注明：个人没有使用过）
...
## 复用
   在实际开发中，会有很多重复性的代码，为了减少不必要的工作和开发周期，提高代码的复用是必不可少的。例如：封装组件。
...
## 路由
   vue 是单页开发，页面跳转不再是后台返回的 url 进行跳转，而是由前端js控制进行跳转的。参考[官方路由](https://router.vuejs.org/)
